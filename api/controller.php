<?php 
if ($_SERVER["HTTP_REFERER"] == "https://athenacryptobank.com/") {
	...
	/*
		...
		case "saldoActual":
			...
			$objUsuario = new usuarioModel();
			$resultaSaldoActual = $objUsuario->saldoActual($request->usuario);

			$resultadoOrdenado = array();
			while($row = mysqli_fetch_array($resultaSaldoActual)){
		 	// crea un objeto donde se incluyen los datos del registro
		   	$objetoPais = array();
		   	$objetoPais["criptomoneda"]       = $row['criptomoneda'];
		   	$objetoPais["inversion"]     		  = $row['inversion'];
